...
module.exports = async function (callback) {
    ...
    // Transfer funds to challenger, proposer already has enough funds from the 1st script
    ...
    await registry.propose("Rectangle", { from: proposer })
    ...
    await registry.challenge("Triangle", { from: challenger })
    ...
    await registry.vote(false, { from: voter3 })

    console.log(`Approve proposal or challenge...\n`)

    // Deployer executes for approval
    await registry.rejectProposal({ from: executor })
    ...
    await registry.redeem(2, { from: voter2 })
    await registry.redeem(2, { from: voter3 })
