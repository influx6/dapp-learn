...
contract NftMarketplace is ReentrancyGuard {
    ...
    // Maps: NFT contract address => (tokenId representing NFT minted by seller when they minted a nft from their contract, each nft minted as a tokenId, seller would generally have minted a number of these and it also can be the tokenId representing the nft some buyer bought from the seller) => List(price, sellers Address)
    mapping(address => mapping(uint256 => Listing)) private s_listings;

    // Maps: Sellers address -> total proceeds from NFT sold from their NFT contract
    ...
    *      recall our main contract again the logic will fail -> in essence do your state changes before calling an external contract, API or oracle
